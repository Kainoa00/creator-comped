      ...
      <div className="w-80 shrink-0 border-r border-white/[0.06] flex flex-col bg-white/5">
        <div className="p-5 border-b border-white/[0.06]">
          ...
          <select
            ...
            className="w-full px-2.5 py-1.5 bg-white/5 border border-white/[0.06] rounded-lg text-xs text-white focus:outline-none"
          >
        ...
        <div className="flex-1 overflow-y-auto">
          {filteredSubmissions.map((sub) => (
            ...
            >
              <div className="flex items-start justify-between mb-1">
                <p className="text-sm font-semibold text-white">{sub.creator}</p>
                <StatusBadge status={sub.status} />
              </div>
              <p className="text-xs text-white/50">{sub.creatorHandle}</p>
              <p className="text-xs text-white/40 mt-0.5">{sub.restaurant}</p>
          ...
          {filteredSubmissions.length === 0 && (
            <div className="py-12 text-center text-white/40 text-sm">No submissions found</div>
          )}
        </div>
      </div>
        ...
        {selectedSubmission ? (
          <div className="p-8 max-w-2xl">
            ...
            <div className="flex items-start justify-between mb-8 border-b border-white/[0.06] pb-6">
              <div>
                <h2 className="text-xl font-black text-white">{selectedSubmission.creator}</h2>
                <p className="text-sm text-white/50">{selectedSubmission.creatorHandle}</p>
                <p className="text-sm text-white/40 mt-0.5">Restaurant: {selectedSubmission.restaurant}</p>
              ...
              <div className="border border-white/[0.06] rounded-lg p-5 bg-[#1a1a1a]">
                ...
                <div className="space-y-3">
                  {selectedSubmission.instagramUrl && (
                    <div className="flex items-center justify-between p-3 bg-white/5 rounded-lg border border-white/[0.06]">
                      ...
                      <a
                        ...
                        className="px-3 py-1.5 rounded-lg bg-white/5 border border-white/[0.06] text-xs font-medium text-white/60 hover:bg-white/10 transition-colors flex items-center gap-1.5"
                      >
                  ...
                  {selectedSubmission.tiktokUrl && (
                    <div className="flex items-center justify-between p-3 bg-white/5 rounded-lg border border-white/[0.06]">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-medium text-white">TikTok Post</p>
                          <p className="text-xs text-white/50 flex items-center gap-1.5 mt-0.5">
                            <Eye className="w-3 h-3" />
